#include "STDEventSaver.h"

#include <vector>
#include <tuple>

ClassImp(STDEventSaver);

STDEventSaver::STDEventSaver()
{

    fName = "STDEventSaver";
}

bool STDEventSaver::Init()
{
    ...
    tree -> Branch("truthTrk", outTruthTrk, "truthTrk[10][5]/D");
    tree -> Branch("SiESum", outSiESum, "SiESum[8][5]/D");
    //////
    // tree -> Branch("Si")
...
void STDEventSaver::Exec(Option_t *option)
{
    outTrkNum = 0;
    ...
    for(int si=0; si<siDetNum; si++){
        cout << siESum[si][4] << "        " ;
        if(siESum[si][4] > threshold[0]){isBeamLike = true;}
        int siTrk = 0;
